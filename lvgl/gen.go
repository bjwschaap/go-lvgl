...
func mapReturnType(t string) string {
	rtm := map[string]string{
		"lv_style_int_t":    "int",
		"lv_opa_t":          "LVOpacity",
		"bool":              "bool",
		"lv_blend_mode_t":   "LVBlendMode",
		...
		"const void *":      "unsafe.Pointer",
		"lv_align_t":        "uint8",
		"const lv_font_t *": "(*LVFont)(unsafe.Pointer",
		"const char *":      "C.GoString",
		"lv_text_decor_t":   "LVTextDecor",
		"lv_anim_path_t *":  "(*LVAnimationPath)(unsafe.Pointer",
	}
...
{{- $rt := mapReturnType $p.ValueType }}
func (obj *LVObj) GetStyle{{- snakeToCamel $p.FuncName -}}(part uint8) {{ $vt }} {
	return {{ $rt }}(C.lv_obj_get_style_{{- $p.FuncName -}}((*C.struct__lv_obj_t)(unsafe.Pointer(obj)), C.uchar(part))){{- if hasPrefix $rt "(*" }}){{- end }}
}

func (style *LVStyle) Set{{- snakeToCamel $p.FuncName -}}(state LVState, value {{ $vt }}) {
	C.lv_style_set_{{- $p.FuncName -}}((*C.struct___7)(unsafe.Pointer(style)), C.uchar(state), {{ mapParamType $p.ValueType }})
}

func (obj *LVObj) SetLocalStyle{{- snakeToCamel $p.FuncName -}}(part uint8, state LVState, value {{ $vt }}) {
